defmodule ModestEx.GetAttribute do
  ...
  def get_attribute([bin|bins], selector, key) when is_bitstring(selector) when is_bitstring(key) do
    get_attribute(bin, selector, key) ++ get_attribute(bins, selector, key)
  end

  def get_attribute([], _, _), do: []
  
  def get_attribute(bin, selector, key) when is_bitstring(bin) when is_bitstring(selector) when is_bitstring(key) do
