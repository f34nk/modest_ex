...
ETERM *handle_prepend(ErlMessage* emsg, ETERM *response){
  if(response == NULL){
    ETERM *append_pattern = erl_format("{prepend, Html, Selector, NewHtml}");;
    ...
    if (erl_match(append_pattern, emsg->msg))
    {
      ...
      const char* result_string = modest_select_and_prepend(html_string, selector_string, new_html_string);
