...
void select_and_replace(html_workspace_t* w, const char* html, const char* selector, const char* new_html, const char* scope_name, eterm_array_t* term_array)
{
...
ETERM* handle_replace(ErlMessage* emsg)
{
  ...
  if (erl_match(pattern, emsg->msg)) {
    ...
    select_and_replace(workspace, html, selector, new_html, scope, term_array);
    ETERM* term_list = eterm_array_to_list(term_array);
    response = erl_format("{replace, ~w}", term_list);
